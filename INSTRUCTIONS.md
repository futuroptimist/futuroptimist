...
Launch a collaborative fact-check thread with
`python src/create_fact_check_discussion.py SLUG` (defaults to posting in the
`futuroptimist/futuroptimist` repository under the "Fact Checks" discussion
category). The CLI reads `video_scripts/<slug>/metadata.json`, builds a Markdown
checklist linking to the draft script and YouTube ID, and authenticates with
`GH_TOKEN`/`GITHUB_TOKEN` via GitHub's GraphQL API. Regression coverage in
`tests/test_create_fact_check_discussion.py` stubs the API calls to verify the
query, mutation, and rendered discussion body so future tweaks stay deterministic.

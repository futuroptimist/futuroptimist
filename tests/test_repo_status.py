...
def _mock_repo_status_requests(
    ...
    *,
    ...
    def fake_get(url: str, headers: dict, timeout: int):
        if url.startswith("https://api.github.com/search/issues?q=repo:"):
            return DummyResp({"total_count": None})
...
def test_fetch_repo_metadata_includes_merged_pr_count(
    ...
    assert repo_status.fetch_repo_metadata("user/repo", token="token") == (
...
@pytest.mark.parametrize("payload", [{}, {"total_count": "42"}, []])
def test_fetch_merged_pr_count_invalid_payload_is_unknown(
    ...
    monkeypatch.setattr(
        repo_status.requests,
        "get",
        lambda url, headers, timeout: DummyResp(payload),
    )
...
def test_update_readme_renders_known_stars_with_known_merged_prs(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    readme = tmp_path / "README.md"
    readme.write_text(
...
def test_update_readme_renders_unknown_stars_with_known_merged_prs(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    readme = tmp_path / "README.md"
    readme.write_text(
    ...
    assert readme.read_text().splitlines()[2] == (
        "- ❓ ⭐ ? 🔀 42 **[repo](https://github.com/user/repo)** - desc"
    )

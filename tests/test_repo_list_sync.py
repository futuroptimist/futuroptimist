...
def test_prompt_repo_lists_match() -> None:
    repos_from = Path("data/prompt-docs/prompt-doc-repos.txt")
    repo_list = Path("docs/repo_list.txt")

    assert repos_from.exists(), "data/prompt-docs/prompt-doc-repos.txt must exist"
    ...
    assert list_entries == repos_entries, (
        "docs/repo_list.txt must match data/prompt-docs/prompt-doc-repos.txt. "

...
* Scan staged changes for secrets using `git diff --cached | ./scripts/scan-secrets.py`
  before committing. Add `# allowlist secret` (any case) to suppress false positives.
